...
def calculate_employee_payroll(employee, year=None, month=None, start_date=None, end_date=None):
    ...
    # Effective payable days worked or on approved paid leave
    payable_days = present_days + paid_leave_days

    # -------------------------------------------------------------------------
    # 26 Standard Working Days Base Rate & Absence Deduction Calculation
    # -------------------------------------------------------------------------
    base_salary = employee.salary or Decimal("0.00")
    STANDARD_WORKING_DAYS = Decimal("26.0")

    if base_salary > Decimal("0.00"):
        # Fixed daily wage rate based on standard 26 working shifts
        daily_rate = (base_salary / STANDARD_WORKING_DAYS).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        
        # Determine total working days missed below standard 26 shifts
        working_days_absent = max(Decimal("0.0"), STANDARD_WORKING_DAYS - payable_days)
        
        # Deduct exact per-day rate for every missed day
        attendance_deduction = (working_days_absent * daily_rate).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
        gross_earned = max(Decimal("0.00"), base_salary - attendance_deduction)
    else:
